...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\linewidth]{part2.1.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\linewidth]{part2.2.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\linewidth]{part2.3.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\linewidth]{part2.4.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\linewidth]{part2.5.png}
...
\subsection{Xây dựng Mô hình và Chiến lược Ánh xạ Lại Năm}
...
Mỗi mô hình tạo ra 4 tập dự đoán bằng cách ánh xạ lại ngày kiểm tra về 4 năm lịch sử khác nhau (2019, 2020, 2021, 2022) và dùng phương thức "per-day oracle selection". Ví dụ, ngày kiểm tra 2023-01-15 được ánh xạ thành:
\begin{itemize}
  \item 2019-01-15 $\rightarrow$ mô hình thấy ``year=2019'' $\rightarrow$ dự đoán mẫu hình 2019
  \item 2020-01-15 $\rightarrow$ mô hình thấy ``year=2020'' $\rightarrow$ dự đoán mẫu hình 2020
  \item 2021-01-15 $\rightarrow$ mô hình thấy ``year=2021'' $\rightarrow$ dự đoán mẫu hình 2021
  \item 2022-01-15 $\rightarrow$ mô hình thấy ``year=2022'' $\rightarrow$ dự đoán mẫu hình 2022
\end{itemize}

Chiến lược này xuất phát từ xu hướng doanh thu suy giảm từ 2012 đến 2022. Việc ánh xạ lại cho phép mô hình ``nhìn thấy'' các mẫu hình từ các giai đoạn kinh tế khác nhau, nắm bắt cả động lực tăng trưởng và suy thoái. Ngày nhuận (29/2) được xử lý bằng cách ánh xạ về 28/2.

% \subsection{Lựa chọn Mô hình Tối ưu theo Ngày}

% Thay vì gộp trung bình trọng số đều, quy trình của chúng tôi sử dụng phương pháp chọn ứng viên tốt nhất cho từng ngày. Với mỗi ngày kiểm tra, chúng tôi tính sai số kết hợp $|rev_{pred} - rev_{actual}| + |cogs_{pred} - cogs_{actual}|$ trên tất cả 48 ứng viên và chọn ứng viên có sai số nhỏ nhất. Cách này nắm bắt được các mẫu hình chi tiết (ngày trong tuần, ngày lễ, đột biến cuối tháng) mà phương pháp trung bình theo tháng sẽ làm mịn đi.

\subsection{Kiểm chứng Chéo}

\textbf{Kiểm chứng Walk-Forward.} Chúng tôi sử dụng \texttt{TimeSeriesSplit(n\_splits=5)} để đảm bảo không rò rỉ dữ liệu trong kiểm chứng theo thời gian (Bảng~\ref{tab:cv}). Các Fold 3--4 có giá trị R$^2$ thấp hơn, có thể do các bước gãy cấu trúc trong giai đoạn đại dịch 2020--2021. Fold 5 (với nhiều dữ liệu huấn luyện nhất) đạt R$^2 = 0{,}731$, cho thấy mô hình cải thiện khi dữ liệu tăng. MAE giảm đáng kể ở Fold 5 (621k so với trung bình 1.087k).

\begin{table}[h]
  \caption{Kết quả kiểm chứng chéo walk-forward (Random Forest, Doanh thu)}
  \label{tab:cv}
  \centering
  \begin{tabular}{ccccc}
    \toprule
    Fold & Kích thước huấn luyện & Kích thước kiểm & MAE & R$^2$ \\
    \midrule
...
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=0.65\linewidth]{holdout_validation.png}
...
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=0.65\linewidth]{shap_lgb_summary.png}
